<div class="flex flex-col h-[calc(100vh-12rem)] max-w-3xl mx-auto">
    {{-- Header --}}
    <div class="flex items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                ...
                <flux:text class="text-emerald-600 dark:text-emerald-400 text-xs sm:text-sm font-medium">Online and ready to help</flux:text>
            </div>
        </div>
        <div class="flex items-center gap-1 sm:gap-2 shrink-0">
            @if (count($messages) > 0)
                <flux:button wire:click="clearChat" variant="subtle" icon="trash" size="sm" class="max-sm:!px-2">
        ...
        @forelse ($messages as $index => $message)
            @if ($message['role'] === 'user')
                ...
                <div wire:key="message-{{ $index }}" class="flex items-start gap-2.5 justify-end pl-8">
                    <div class="bg-zinc-100 dark:bg-zinc-700 rounded-2xl rounded-br-md px-4 py-2.5 border border-zinc-200 dark:border-zinc-600">
            ...
            @else
                ...
                <div wire:key="message-{{ $index }}" class="flex items-start gap-2.5 pr-8">
                    ...
                    <div class="bg-amber-50/50 dark:bg-amber-950/20 rounded-2xl rounded-bl-md px-4 py-2.5 border border-amber-200/50 dark:border-amber-800/30">
        ...
        @empty
            ...
            <div class="flex items-center justify-center h-full min-h-64 px-4">
                <div class="text-center max-w-sm">
                    ...
                    <flux:heading size="lg" class="mb-2">Hi, I'm your Skills Coach</flux:heading>
                    <flux:text class="text-zinc-500 dark:text-zinc-400 text-sm mb-5">
                        Ask me about skills to learn, career paths, or how to level up your expertise.
                    </flux:text>
                    <div class="flex flex-wrap gap-2 justify-center">
                        <span class="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs sm:text-sm rounded-full cursor-pointer hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors">What should I learn?</span>
                        <span class="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs sm:text-sm rounded-full cursor-pointer hover:bg-amber-200 dark:hover:bg-amber-900/50 transition-colors">Review my skills</span>
                    </div>
                </div>
            </div>
        @endforelse
